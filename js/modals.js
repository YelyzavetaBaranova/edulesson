...
export function openCF() {
  openMO(`<div class="mb-scroll"><div class="m-title">Новий курс</div>
...
export function openCL(courseId) {
  openMO(`<div class="mb-scroll"><div class="m-title">Новий урок</div>
    ...
    <div class="m-footer"><button class="btn bg" data-action="close-modal">Скасувати</button><button class="btn bp" data-action="create-lesson" data-fid="${courseId}">Створити</button></div>`);
  setTimeout(() => document.getElementById('mLname')?.focus(), 50);
}

export function openCT(courseId, lessonId) {
  state.setTType('choose');
  state.setPendingImages([]);
  state.setPendingCaptions([]);
  state.setPendingHintImg(null);
  openMO(buildTM(courseId, lessonId, null));
}

export function buildTM(courseId, lessonId, ex) {
  ...
  return `
    ...
    <div class="type-grid">
      ${[
        ...
        .map(
          ([tp, ico, nm, sb]) => `
        <div class="type-opt ${state.tType === tp ? 'sel' : ''}" data-action="sel-type" data-type="${tp}" data-fid="${courseId}" data-lid="${lessonId}" data-eid="${ex ? ex.id : ''}">
    ...
    <div class="m-footer">
      ...
      <button class="btn bp" data-action="${ex ? 'save-edit-task' : 'save-new-task'}" data-fid="${courseId}" data-lid="${lessonId}" data-tid="${ex ? ex.id : ''}">${ex ? 'Зберегти' : 'Додати'}</button>
    </div>`;
}
...
export function editTask(courseId, lessonId, taskId) {
  ...
  openMO(buildTM(courseId, lessonId, task));
...
export async function saveNewTask(courseId, lessonId) {
  ...
  closeMO();
  const n = await nav();
  n.openLesson(courseId, lessonId);
}
...
export async function createLesson(courseId) {
  ...
  n.renderSB();
  n.openLesson(courseId, (getCourseRef(courseId)?.lessons || []).slice(-1)[0]?.id);
}
