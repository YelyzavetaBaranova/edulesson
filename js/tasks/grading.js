...
export function checkOne(tid, type) {
  ...
  if (type === 'choose') {
    ...
    let all = true;
    groups.forEach((g) => {
      if (!g.dataset.chosen) { all = false; return; }
    ...
    inputs.forEach((inp) => {
      ...
      else { inp.classList.add('bad'); all = false; }
    });
...
export function resetTask(tid, type) {
  ...
  card.classList.remove('c-ok', 'c-bad');
  const fb = document.getElementById(`fb-${tid}`);
  if (fb) { fb.className = 'task-fb'; fb.innerHTML = ''; }
  const saBtn = document.getElementById(`sa-${tid}`);
  if (saBtn) saBtn.style.display = 'none';
...
export async function checkAll(courseId, lessonId) {
  ...
  if (total > 0 && correct >= total) {
    await updateProgress(lessonId, 'done');
  }
}

export async function resetAll(courseId, lessonId) {
